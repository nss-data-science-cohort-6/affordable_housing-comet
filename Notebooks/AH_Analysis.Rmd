...
Filter barnes and LIHTC to coordinates and year put into services, so we can 
stack them.
```{r}
LIHTC <- LIHTC %>% 
...
```{r}
simple_regression_model <- lm(amount ~ square_footage + age + group + ownerdate + tract,
                              data = nearest_ah)
...
```{r}
nearest_ah <- nearest_ah %>% 
  mutate(group = factor(group), tract = factor(tract), tpost = factor(tpost))



simple_regression_model_log <- lm(log(amount) ~ square_footage + age + 
                                    relevel(group, ref = 'outside') + ownerdate + tract + tpost,
                                  data = nearest_ah)
