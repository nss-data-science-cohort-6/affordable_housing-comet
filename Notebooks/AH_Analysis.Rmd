...
```{r}
#First get sell date info for all the properties in property_details_sf from filtered_sales by matching apn values in each DF and merging
merged_property_sales <- merge(x = property_details_sf,
                               y = filtered_sales,
                               by = "apn"
                              ) %>% 
                                distinct()#Remove repeated data rows
...
merged_property_sales %>% 
                      mutate(
                        group = case_when(
                                          (dis_from_LIHTC_miles < 0.5 && ... ) ~ "pre",
                                          (dis_from_LIHTC_miles < 0.5 && ... ) ~ "mid",
                                          (dis_from_LIHTC_miles < 0.5 && ... ) ~ "post",
                                          TRUE ~ "other"
                                          )
                            )
```
